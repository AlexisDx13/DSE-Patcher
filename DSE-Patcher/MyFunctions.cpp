...
int MyGetg_CiOptionsKernelAddress(UINT64 ui64ImageBase,UINT64 *ui64Kernelg_CiOptions,DWORD dwBuildNumber)
{
    ...
    *ui64Kernelg_CiOptions = 0;
    ...
    else if(dwBuildNumber >= 16299 && dwBuildNumber < 26100)
    {
        ...
        for (ULONG i = 0; i < 0x6E; i += hs.len)
        {
            ...
            else if (ulInstructionsFound == 4 && hs.len == 5 && CiInitialize[i] == 0xE8)
            {
                ...
                // Windows 10 x64 Version 1709 / Windows 11 22h2 x64 22621
            ...
            {
                // Reset and re-check if the current instruction is the start of the sequence.
                // This handles cases where a partial match is broken by an instruction
                // that is itself the start of the sequence (e.g., A, A, B, C).
    ...
    // Windows 11 24H2 (dwBuildNumber = 26100)
    else if(dwBuildNumber >= 26100)
    {
        ...
        for(ULONG i = 0; i < 0x6E; i += hs.len)
        {
            ...
            // 2nd we search for the move instruction "mov r8d, edi" with a length of 3 bytes
            //lint -e{679} Warning 679:Suspicious Truncation in arithmetic expression combining with pointer
            else if(ulInstructionsFound == 1 && hs.len == 3 && CiInitialize[i] == 0x44 && CiInitialize[i + 1] == 0x8B && CiInitialize[i + 2] == 0xC7)
            {
                ulInstructionsFound = 2;
            }
            // 3rd we search for the move instruction "mov [rsp+20h], rax" with a length of 5 bytes
            //lint -e{679} Warning 679:Suspicious Truncation in arithmetic expression combining with pointer
            ...
            // 4th we search for the move instruction "mov rdx, rsi" with a length of 3 bytes
            //lint -e{679} Warning 679:Suspicious Truncation in arithmetic expression combining with pointer
            ...
            {
                // Windows 11 24H2 x64 Version 26100
                // PAGE:00000001C0050A50 ; Exported entry  10. CiInitialize
                // PAGE:00000001C0050A50
                // PAGE:00000001C0050A50 ; =============== S U B R O U T I N E =======================================
                // PAGE:00000001C0050A50
                // PAGE:00000001C0050A50
                // PAGE:00000001C0050A50                 public CiInitialize
                // PAGE:00000001C0050A50 CiInitialize    proc near               ; DATA XREF: .pdata:00000001C0044920↑o
                // PAGE:00000001C0050A50
                // PAGE:00000001C0050A50 var_18          = qword ptr -18h
                // PAGE:00000001C0050A50 arg_0           = qword ptr  8
                // PAGE:00000001C0050A50 arg_8           = qword ptr  10h
                // PAGE:00000001C0050A50 arg_10          = qword ptr  18h
                // PAGE:00000001C0050A50 arg_20          = qword ptr  28h
                // PAGE:00000001C0050A50
                // PAGE:00000001C0050A50                 mov     [rsp+arg_0], rbx
                // PAGE:00000001C0050A55                 mov     [rsp+arg_8], rbp
                // PAGE:00000001C0050A5A                 mov     [rsp+arg_10], rsi
                // PAGE:00000001C0050A5F                 push    rdi
                // PAGE:00000001C0050A60                 sub     rsp, 30h
                // PAGE:00000001C0050A64                 mov     rbx, r9
                // PAGE:00000001C0050A67                 mov     edi, r8d
                // PAGE:00000001C0050A6A                 mov     rsi, rdx
                // PAGE:00000001C0050A6D                 mov     ebp, ecx
                // PAGE:00000001C0050A6F                 call    sub_1C00EA008
                // PAGE:00000001C0050A74                 mov     rax, [rsp+38h+arg_20]
                // PAGE:00000001C0050A79                 mov     r9, rbx
                // PAGE:00000001C0050A7C                 mov     r8d, edi
                // PAGE:00000001C0050A7F                 mov     [rsp+38h+var_18], rax
                // PAGE:00000001C0050A84                 mov     rdx, rsi
                // PAGE:00000001C0050A87                 mov     ecx, ebp
                // PAGE:00000001C0050A89                 call    sub_1C0051C30          <---
                // PAGE:00000001C0050A8E                 mov     rbx, [rsp+38h+arg_0]
                // PAGE:00000001C0050A93                 mov     rbp, [rsp+38h+arg_8]
                // PAGE:00000001C0050A98                 mov     rsi, [rsp+38h+arg_10]
                // PAGE:00000001C0050A9D                 add     rsp, 30h
                // PAGE:00000001C0050AA1                 pop     rdi
                // PAGE:00000001C0050AA2                 retn
                // PAGE:00000001C0050AA2 ; ---------------------------------------------------------------------------
                // PAGE:00000001C0050AA3                 db 0CCh
                // PAGE:00000001C0050AA3 CiInitialize    endp
    ...
    for(ULONG i = 0; i < 0x4A; i += hs.len)
    {
        ...
        //lint -e{679} Warning 679:Suspicious Truncation in arithmetic expression combining with pointer
        if(hs.len == 6 && CipInitialize[i] == 0x89 && (CipInitialize[i + 1] == 0x0D || CipInitialize[i + 1] == 0x05))
        {
            ...
            // Windows 11 22h2 x64 22621
            // PAGE:00000001C0045028 sub_1C0045028   proc near               ; CODE XREF: CiInitialize+2F↑p
            // PAGE:00000001C0045028                                         ; DATA XREF: .pdata:00000001C003A29C↑o
            // PAGE:00000001C0045028
            // PAGE:00000001C0045028 var_38          = qword ptr -38h
            // PAGE:00000001C0045028 var_30          = dword ptr -30h
            // PAGE:00000001C0045028 var_28          = qword ptr -28h
            // PAGE:00000001C0045028 var_20          = dword ptr -20h
            // PAGE:00000001C0045028 arg_0           = qword ptr  8
            // PAGE:00000001C0045028 arg_8           = qword ptr  10h
            // PAGE:00000001C0045028 arg_10          = qword ptr  18h
            // PAGE:00000001C0045028
            // PAGE:00000001C0045028                 mov     [rsp+arg_0], rbx
            // PAGE:00000001C004502D                 mov     [rsp+arg_8], rbp
            // PAGE:00000001C0045032                 mov     [rsp+arg_10], rsi
            // PAGE:00000001C0045037                 push    rdi
            // PAGE:00000001C0045038                 push    r12
            // PAGE:00000001C004503A                 push    r14
            // PAGE:00000001C004503C                 sub     rsp, 40h
            // PAGE:00000001C0045040                 mov     rbp, r9
            // PAGE:00000001C0045043                 mov     cs:dword_1C0043004, ecx
            // PAGE:00000001C0045049                 mov     rbx, r8
            // PAGE:00000001C004504C                 mov     rsi, rdx
            // PAGE:00000001C004504F                 mov     r14d, ecx
            // PAGE:00000001C0045052                 call    cs:PsGetCurrentProcess
            // PAGE:00000001C0045059                 nop     dword ptr [rax+rax+00h]
            // PAGE:00000001C004505E                 mov     cs:PROCESS, rax
            // PAGE:00000001C0045065                 cmp     dword ptr [rbx], 0F8h
            // PAGE:00000001C004506B                 jz      short loc_1C0045077
            // PAGE:00000001C004506D                 mov     eax, 0C00000F1h
            // PAGE:00000001C0045072                 jmp     loc_1C00453CE

            // Win 11 24h2 x64 Version 26100
            // PAGE:00000001C0051C30 sub_1C0051C30   proc near               ; CODE XREF: CiInitialize+39↑p
            // PAGE:00000001C0051C30                                         ; DATA XREF: .pdata:00000001C00449B0↑o
            // PAGE:00000001C0051C30
            // PAGE:00000001C0051C30 arg_0           = qword ptr  8
            // PAGE:00000001C0051C30 arg_8           = qword ptr  10h
            // PAGE:00000001C0051C30 arg_10          = qword ptr  18h
            // PAGE:00000001C0051C30 arg_20          = qword ptr  28h
            // PAGE:00000001C0051C30
            // PAGE:00000001C0051C30                 mov     [rsp+arg_0], rbx
            // PAGE:00000001C0051C35                 mov     [rsp+arg_8], rbp
            // PAGE:00000001C0051C3A                 mov     [rsp+arg_10], rsi
            // PAGE:00000001C0051C3F                 push    rdi
            // PAGE:00000001C0051C40                 push    r12
            // PAGE:00000001C0051C42                 push    r13
            // PAGE:00000001C0051C44                 push    r14
            // PAGE:00000001C0051C46                 push    r15
            // PAGE:00000001C0051C48                 sub     rsp, 20h
            // PAGE:00000001C0051C4C                 mov     rbx, r9
            // PAGE:00000001C0051C4F                 mov     edi, r8d
            // PAGE:00000001C0051C52                 mov     rbp, rdx
            // PAGE:00000001C0051C55                 mov     cs:dword_1C004E004, ecx   <--- g_CiOptions (dword_1C004E004)
            // PAGE:00000001C0051C5B                 mov     r13d, ecx
            // PAGE:00000001C0051C5E                 call    cs:PsGetCurrentProcess
            // PAGE:00000001C0051C65                 nop     dword ptr [rax+rax+00h]
            // PAGE:00000001C0051C6A                 mov     cs:PROCESS, rax
            // PAGE:00000001C0051C71                 cmp     dword ptr [rbx], 100h
            // PAGE:00000001C0051C77                 jz      short loc_1C0051C83
            // PAGE:00000001C0051C79                 mov     eax, 0C00000F1h
            // PAGE:00000001C0051C7E                 jmp     loc_1C0051FD7
...
//------------------------------------------------------------------------------
static int MyEnableDebugPrivilege()
{
    HANDLE hToken = NULL;
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
        return 1; // Cannot open process token
    }
    ...
    if (!LookupPrivilegeValue(NULL, SE_DEBUG_NAME, &luid)) {
        CloseHandle(hToken);
        return 2; // Cannot lookup privilege value
    }
    ...
    if (!AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), (PTOKEN_PRIVILEGES)NULL, (PDWORD)NULL) || GetLastError() == ERROR_NOT_ALL_ASSIGNED) {
        CloseHandle(hToken);
        return 3; // Cannot adjust token privileges or privilege not held
    }
...
DWORD WINAPI MyThreadProc1(PVOID pvoid)
{
    ...
    SendMessage(g.Dlg1.hStatusBar1, SB_SETTEXT, (WPARAM)0, (LPARAM)"Enabling required privileges...");
    if (MyEnableDebugPrivilege() != 0) {
        MessageBox(g.Dlg1.hDialog1, "Could not enable SeDebugPrivilege!\nPlease ensure the application is running as an Administrator.", "Privilege Error", 16);
        // Use a unique error code to distinguish from others
        rc = 20;
        goto cleanup;
    }
